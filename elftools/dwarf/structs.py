...
from construct import (
    ...
    Struct, Switch, Computed, Padding, PascalString, NullTerminated, GreedyBytes
)
from ..common.construct_utils import (
    RepeatUntilExcluding, ULEB128, SLEB128, StreamOffset, EmbeddableStruct, Embed,
    CStringBytes
)
from .enums import *
...
class DWARFStructs(object):
    ...
    def _create_abbrev_declaration(self):
        self.Dwarf_abbrev_declaration = Struct(  # Dwarf_abbrev_entry
            ...
            'attr_spec' / RepeatUntilExcluding(lambda obj, list, ctx: obj.name == 'DW_AT_null' and obj.form == 'DW_FORM_null',
                Struct(
    ...
    def _create_lineprog_header(self):
        ...
        self.Dwarf_lineprog_header = EmbeddableStruct(
            ...
            Embed(IfThenElse(ver5,
                ...
                Struct(
                    'include_directory' / RepeatUntilExcluding(lambda obj, lst, ctx: obj == b'',
                                                               NullTerminated(GreedyBytes)),
                    'file_entry' / RepeatUntilExcluding(lambda obj, lst, ctx: len(obj.name) == 0,
    ...
    def _create_loclists_parsers(self):
        ...
        self.Dwarf_loclists_CU_header = Struct(
            'cu_offset' / StreamOffset(),
            'unit_length' / self.Dwarf_initial_length,
            'is64' / Computed(lambda ctx: ctx.is64),
            'offset_after_length' / StreamOffset(),
            'version' / self.Dwarf_uint16,
            'address_size' / self.Dwarf_uint8,
            'segment_selector_size' / self.Dwarf_uint8,
            'offset_count' / self.Dwarf_uint32,
            'offset_table_offset' / StreamOffset()
        )
        ...
        self.Dwarf_loclists_entries = RepeatUntilExcluding(
            lambda obj, list, ctx: obj.entry_type == 'DW_LLE_end_of_list',
            EmbeddableStruct(
                'entry_offset' / StreamOffset(),
    ...
    def _create_rnglists_parsers(self):
        ...
        self.Dwarf_rnglists_entries = RepeatUntilExcluding(
            lambda obj, list, ctx: obj.entry_type == 'DW_RLE_end_of_list',
            EmbeddableStruct(
                'entry_offset' / StreamOffset(),
